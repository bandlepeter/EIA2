...
(function (L09) {
    ...
    let letter;
    let abc = [];
    let temp; //Speichert temporär den Inhalt der ausgewählten Buchstaben
    function init() {
    ...
    function pressMouseField(_event) {
        if (temp == "")
            return;
        ...
        //lösche temp wieder
        let clickedDiv = document.getElementById(temp.toLowerCase());
        clickedDiv.style.color = "#ffffff";
        clickedDiv.style.backgroundColor = "#d3d3d3";
        temp = "";
    }
    function keydown1(_event) {
        if (abc.indexOf(_event.key.toUpperCase()) != -1) {
